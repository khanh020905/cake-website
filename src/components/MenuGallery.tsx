...
export const MenuGallery = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  const [activeCategory, setActiveCategory] = useState('all');
  const [searchQuery, setSearchQuery] = useState('');
  const [visibleCount, setVisibleCount] = useState(12);
  const [isCategoryLoading, setIsCategoryLoading] = useState(false);

  // Progressive Rendering: Reset chunk size on filter change to avoid 'khựng'
  useEffect(() => {
    setVisibleCount(12);
  }, [activeCategory, searchQuery]);

  // Progressive Rendering: Load remaining items in the background
  useEffect(() => {
    if (isOpen) {
      if (visibleCount < menuData.length) {
        const timer = setTimeout(() => {
          setVisibleCount((prev) => prev + 12);
        }, 150); // 150ms delay chunking keeps UI responsive
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[100] bg-[#FAFAFA] flex flex-col md:flex-row overflow-hidden"
        >
          {/* Header/Close Button (Mobile) */}
          <div className="absolute top-6 right-6 z-50">
            <button 
              onClick={onClose}
              className="p-3 bg-white hover:bg-black hover:text-white transition-colors duration-300 rounded-full shadow-lg border border-[#F4B5B7]/30"
            >
              <X size={24} />
            </button>
          </div>
          ...
          <div className="w-full md:w-80 lg:w-96 flex-shrink-0 bg-white border-b md:border-b-0 md:border-r border-[#F4B5B7]/20 p-8 flex flex-col justify-between overflow-y-auto h-auto md:h-full z-10 pt-20 md:pt-16 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
            <div>
              <motion.div 
          ...
          {/* Grid View */}
          <div className="flex-1 overflow-y-auto bg-[#FAFAFA] p-4 md:p-12 lg:p-16 h-full relative" id="menu-grid-container">
            {isCategoryLoading ? (
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="w-full h-full min-h-[50vh] flex flex-col items-center justify-center text-center"
              >
                 <motion.div 
                    animate={{ rotate: 360 }}
